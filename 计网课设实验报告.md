...
在程序运行在初始状态是，如果用户在客户端按下了上传文件按钮，就相当于在Terminal中输入一条`upload`指令，当输入了`upload`指令，客户端会弹出文件选择界面，文件选择器会过滤出所有的Verilog文件，然后，客户端会进入上传文件状态，首先客户端将上传命令发给服务器端，然后客户端等待服务器端的响应，当客户端接受到服务器端的回应后，会开始运行上传程序，上传程序的内容主要是不断地读取文件内容，并且向服务器端程序发送文件内容，当文件发送完毕后，程序会在最后一行补上EREQ标识，代表发送完毕。然后程序等待服务器地响应，确认发送成功后，程序会发送编译命令，要求服务器端将刚才上传地Verilog文件通过服务器端的Verilator程序编译成可以执行地Verilog模型，客户端读取程序发送来地结果，如果发现结果是错误的，则会禁用FPGA的运行按钮，因为此时没有可执行的Verilog模型，无法运行，需要用户上传一个正确的可用的Verilog文件才能继续运行程序。
...
![image-20220328103353471](F:\GIT\NetDesign\NetDesign\计网课设实验报告.assets\image-20220328103353471.png)

成功连接时，服务器端显示

![image-20220328104624269](F:\GIT\NetDesign\NetDesign\计网课设实验报告.assets\image-20220328104624269.png)

客户端显示

![image-20220327173636137](F:\GIT\NetDesign\NetDesign\计网课设实验报告.assets\image-20220327173636137.png)

客户端关闭窗口会断开连接

服务器端显示

![image-20220328104840706](F:\GIT\NetDesign\NetDesign\计网课设实验报告.assets\image-20220328104840706.png)

当再次启动客户端程序

服务器端显示：

![image-20220328104943792](F:\GIT\NetDesign\NetDesign\计网课设实验报告.assets\image-20220328104943792.png)

同时在客户端机器上再运行一个实例，可以看到新的实例也连接到了服务器。

![image-20220328105101272](F:\GIT\NetDesign\NetDesign\计网课设实验报告.assets\image-20220328105101272.png)

两个客户端程序都可以工作

![image-20220328105226389](F:\GIT\NetDesign\NetDesign\计网课设实验报告.assets\image-20220328105226389.png)

b) terminal测试

ifconfig指令

![image-20220327163003028](F:\GIT\NetDesign\NetDesign\计网课设实验报告.assets\image-20220327163003028.png)

ping指令

![image-20220327163652785](F:\GIT\NetDesign\NetDesign\计网课设实验报告.assets\image-20220327163652785.png)

gcc -v指令

![image-20220327163605377](F:\GIT\NetDesign\NetDesign\计网课设实验报告.assets\image-20220327163605377.png)

ustb (并不存在的指令)

![image-20220327163719330](F:\GIT\NetDesign\NetDesign\计网课设实验报告.assets\image-20220327163719330.png)

c) 上传并运行文件

1. 文件一

   ![image-20220327185047874](F:\GIT\NetDesign\NetDesign\计网课设实验报告.assets\image-20220327185047874.png)

   

   ![image-20220327185233225](F:\GIT\NetDesign\NetDesign\计网课设实验报告.assets\image-20220327185233225.png)   

   

2. 文件二

![image-20220327202815290](F:\GIT\NetDesign\NetDesign\计网课设实验报告.assets\image-20220327202815290.png)

![image-20220327202837307](F:\GIT\NetDesign\NetDesign\计网课设实验报告.assets\image-20220327202837307.png)

3. 文件三

   ![image-20220327202916101](F:\GIT\NetDesign\NetDesign\计网课设实验报告.assets\image-20220327202916101.png)

   ![image-20220327202927425](F:\GIT\NetDesign\NetDesign\计网课设实验报告.assets\image-20220327202927425.png)

   ![image-20220327203339250](F:\GIT\NetDesign\NetDesign\计网课设实验报告.assets\image-20220327203339250.png)

   ![image-20220327203431333](F:\GIT\NetDesign\NetDesign\计网课设实验报告.assets\image-20220327203431333.png)
...
当前的远程控制程序暴露了过多的权限给用户，在安全性上有着较大的缺陷，远程用户可以执行大多数shell程序，同时对连接没有验证，因此安全性上较为堪忧。后续可以针对这一点做出改进
